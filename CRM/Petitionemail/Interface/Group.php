    ...
    foreach ($groupIds as $groupId) {
      ...
      foreach ($groupContacts['values'] as $groupContact) {
        ...
        if (!CRM_Utils_Mail::send($mailParams)) {
          CRM_Core_Session::setStatus(ts('Error sending message to %1', array('domain' => 'com.jlacey.petitionemailgroup', 1 => $mailParams['toName'])));
        }
        else {
          CRM_Core_Session::setStatus(ts('Message sent successfully to %1', array('domain' => 'com.jlacey.petitionemailgroup', 1 => $mailParams['toName'])));
        }
